public class mountainArraySearch {
    public static int findInMountainArray(int target,int[] mountianArr){
        ...
        int firsttry=orderAgnosticBS(mountianArr,0,peak,target);//searching from start to peak
        if (firsttry!=-1){
            return firsttry;
        }else{//searching from peak+1 to end
    ...
    public static int orderAgnosticBS(int[] arr ,int start,int end,int target){
        //IMP - order Agnostic BS is important here , we cannot directly use normal binary search 
        boolean isasc;
